      ...
      <div className="sm:flex sm:items-center sm:justify-between px-4 sm:px-0">
        ...
        <div className="flex items-center gap-4 mt-4 sm:mt-0">
          ...
          <button 
            ...
            className={`inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold shadow-lg transition-all ${
              saveStatus === 'success' 
              ...
              <div className="flex overflow-x-auto border-b border-slate-200 bg-slate-50 custom-scrollbar hide-scroll-bar">
                {event.dailyMenus.map(day => (
                  <button
                    ...
                    className={`min-w-[120px] px-6 py-4 text-sm font-bold border-b-2 whitespace-nowrap transition-colors ${
                      activeTabId === day.id 
...
export function PublicMenu() {
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!profile) {
      alert("Error: No se pudo encontrar el número de contacto del negocio.");
      return;
    }
    ...
    try {
      setLoading(true);
      // Double Trigger: 1. Save to Firestore
             ...
             <div className="flex overflow-x-auto border-b border-slate-50 bg-slate-50/50 hide-scrollbar scroll-smooth">
                {event.dailyMenus.map(day => (
                  <button
                    key={day.id}
                    onClick={() => setActiveTabId(day.id)}
                    className={`flex-1 min-w-[100px] px-4 py-5 text-center transition-all duration-300 ${
                      activeTabId === day.id 
